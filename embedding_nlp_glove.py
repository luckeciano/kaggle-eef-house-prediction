...
filepath="best-models/glove_best"
...
np.savetxt('features-embeddings/embbedings_glove_features.csv', features_train, delimiter=',', fmt=['%.10f'])

y_lb = model.predict(X_val, batch_size=2048)

print (id_val)
res = np.hstack([id_val, y_lb])
np.savetxt('validation/embeddings_glove_validation.csv', res, delimiter=',', fmt=['%d', '%.10f'])
