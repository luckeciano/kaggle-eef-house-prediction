...
EMBEDDING_FILE = 'nlp-embeddings/glove_s600.txt'
...
filepath="best-models/glove_best_300"
...
np.savetxt('features-embeddings/embedding_nlp_glove_300_features.csv', features_train, delimiter=',', fmt=['%.10f'])

y_lb = model.predict(X_val, batch_size=2048)

print (id_val)
res = np.hstack([id_val, y_lb])
np.savetxt('validation/embedding_nlp_glove_300_validation.csv', res, delimiter=',', fmt=['%d', '%.10f'])
