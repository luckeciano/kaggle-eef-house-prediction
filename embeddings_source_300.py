...
file_path="embeddings_source_300"
...
np.savetxt('features-embeddings/embeddings_source_300_features.csv', features_train, delimiter=',', fmt=['%.10f'])

y_lb = model.predict(X_te, batch_size=2048)

print (id_lb)
res = np.hstack([id_lb, y_lb])
np.savetxt('validation/embeddings_source_300_validation.csv', res, delimiter=',', fmt=['%d', '%.10f'])
