...
file_path='best-models/lstm_title_house_pred'

tbCallBack = TensorBoard(log_dir='./folder_' + file_path, histogram_freq=0, write_graph=True, write_images=True)
...
# np.savetxt('features-embeddings/embeddings_source_title_features.csv', features_train, delimiter=',', fmt=['%.10f'])

y_lb = model.predict(X_te, batch_size=2048)

print (id_lb)
res = np.hstack([id_lb, y_lb])
np.savetxt('validation/embeddings_source_title_validation.csv', res, delimiter=',', fmt=['%d', '%.10f'])
