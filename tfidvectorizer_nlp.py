...
def baseline_model():
  ...
  model.load_weights('best-models/tfidfvectorizer_model')
  return model
...
estimator.fit(train_features, y,  validation_split = 0.1, verbose=True)
estimator.model.load_weights('best-models/tfidfvectorizer_model')
y_lb = np.expm1(estimator.predict(val_features)).reshape(id_val.shape)


print (id_val)
res = np.hstack([id_val, y_lb])
np.savetxt('validation/tfidfvectorizer_validation.csv', res, delimiter=',', fmt=['%d', '%.10f'])


# features = estimator.predict(train_features)
# np.savetxt('tfidfvectorizer_features.csv', features, delimiter=',', fmt=['%.10f'])
